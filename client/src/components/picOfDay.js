...
class picOfDay extends Component {
    ...
    getPhotoByDate = date => {
      fetch(`https://api.nasa.gov/planetary/apod?date=${date}&api_key=WcS6o09hGpHUcrgp2AvmWdHRsj2Vlc4GKkd1Zbyr`)
        .then((response) => {
    ...
    componentDidMount(){
      fetch(`https://api.nasa.gov/planetary/apod?api_key=WcS6o09hGpHUcrgp2AvmWdHRsj2Vlc4GKkd1Zbyr`)
        .then((response) => {
          return response.json()
        })
        .then((json) => {
...
<div style={{ zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
    ...
    />
    <div style={{ display: 'flex', alignItems: 'flex-start', border: '5px solid #FFFFFF', borderRadius: '10px', padding: '10px', margin: '20px 0', backgroundColor: '#e2e7f2' }}>
      {this.state.photo && (
        <>
